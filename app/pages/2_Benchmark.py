...
if user_off and user_on:
    try:
        ...
        with st.expander("🧠 Harness Memory Performance", expanded=True):
            ...
            | **Memory Assisted Successes** | `{mem_stats['memory_assisted_successes']}` |
            ...
            | **Challenge Success Rate** | `{mem_stats['challenge_success_rate'] * 100:.1f}%` |
            ''')
            
            if mem_stats['top_learned_strategies']:
                st.markdown("**Top Learned Strategies (Global):**")
                for idx, s in enumerate(mem_stats['top_learned_strategies'], 1):
                    st.markdown(f"**{idx}.** {s['strategy']}<br>&nbsp;&nbsp;&nbsp;&nbsp;*Success:* **{int(s['success_rate']*100)}%**", unsafe_allow_html=True)
            else:
